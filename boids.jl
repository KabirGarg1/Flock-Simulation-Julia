...
# seting number of actors (boids)

n = 200    # 200

# selecting random starting points

x = rand(20:5:(WIDTH - 20), n)
y = rand(20:5:(HEIGHT - 20), n)

# defining initial state of actors
...
# selecting random colors for actors
...
# selecting random initial velocities
...
# creating steering force vectors for rules
...
function flock()
    ...
    separation_force_x = []
    separation_force_y = []
    # initializin empty arrays for alignment rule
    neighbor_vx = []
    neighbor_vy = []
    # initializing empty arrays for cohesion rule
    neighbor_x = []
    neighbor_y = []
    ...
    for i in 1:n
        for j in 1:n
            # searching for boids within perception_radius
            d = distance(boid[i], boid[j])
            if boid[i] !== boid[j] && d < perception_radius
                # populating arrays for separation rule
                ...
                # populating arrays for alignment rule
                push!(neighbor_vx, vx[j])
                push!(neighbor_vy, vy[j])
                # populating arrays for cohesion rule
...
# updating position of actors (boids)
...
function update(g::Game)
    ...
    # clearing steering force vectors
    rx1 = zeros(n)
    ry1 = zeros(n)
    rx2 = zeros(n)
    ry2 = zeros(n)
